...
with db.dbconnect(
    ...
    getsupportquery = (
        """ SELECT supportparams_id FROM supportparams WHERE document_path = '"""
        + basedir
        + unaligned
        + """support.txt' """
    )
    ...
    if not indbsupport:
        ...
        insertdict = {
            "document_path": basedir + unaligned + "support.txt",
            "systempid": Systempid,
            "systemos": Systemos,
            "systemperlv": Systemperlv,
            "systemperlexe": Systemperlexe,
            "idstring": Idstring,
            "program": Program,
            "commandline": commandline,
            "sampleconfig_path": samplesheet,
            "sampleconfig": SampleSheet,
            "time": now,
        }
    ...
    getdatasquery = (
        """ SELECT datasource_id FROM datasource WHERE document_path = '"""
        + demultistats
        + """' """
    )
    print getdatasquery
    ...
    if not indbdatas:
        ...
        insertdict = {
            "document_path": demultistats,
            "runname": runname,
            "rundate": rundate,
            "machine": machine,
            "supportparams_id": supportparamsid,
            "server": servername,
            "time": now,
        }
    ...
    getflowcellquery = (
        """ SELECT flowcell_id FROM flowcell WHERE flowcellname = '""" + fc + """' """
    )
    ...
    getdemuxquery = (
        """ SELECT demux_id FROM demux WHERE flowcell_id = '"""
        + str(flowcellid)
        + """' 
                      AND basemask = '"""
        + bmask
    ...
    print "Demux with " + bmask + " from Flowcell: " + fc + " exists in DB with demux_id: " + str(
        demuxid
    )
    ...
    for row in rows:
        ...
        project = unicode(cols[6].string).encode("utf8")
        getsamplequery = (
            """ SELECT sample_id FROM sample WHERE samplename = '"""
            + samplename
            + """' 
                         AND barcode = '"""
            + barcode
    ...
    for row in rows:
        ...
        getunalquery = (
            """ SELECT unaligned_id FROM unaligned WHERE sample_id = '"""
        ...
        if not indbunal:
            ...
            insertdict = {
                "sample_id": samples[samplename],
                "demux_id": str(demuxid),
                "lane": lane,
                "yield_mb": yield_mb,
                "passed_filter_pct": passed_filter_pct,
                "readcounts": Readcounts,
                "raw_clusters_per_lane_pct": raw_clusters_per_lane_pct,
                "perfect_indexreads_pct": perfect_indexreads_pct,
                "q30_bases_pct": q30_bases_pct,
