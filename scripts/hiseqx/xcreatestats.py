...
def get_summary(tree):
    ...
    * pf Q Score
    ...
    pf_clusters = xpathsum(
        tree,
        ".//Project[@name='all']/Sample[@name='all']/Barcode[@name='all']//Pf/ClusterCount",
    )

    pf_yield = xpathsum(
        tree,
        ".//Project[@name='all']/Sample[@name='all']/Barcode[@name='all']//Pf/Read/Yield",
    )
    pf_read1_yield = xpathsum(
        tree,
        ".//Project[@name='all']/Sample[@name='all']/Barcode[@name='all']//Pf/Read[@number='1']/Yield",
    )
    pf_read2_yield = xpathsum(
        tree,
        ".//Project[@name='all']/Sample[@name='all']/Barcode[@name='all']//Pf/Read[@number='2']/Yield",
    )
    ...
    pf_q30 = xpathsum(
        tree,
        ".//Project[@name='all']/Sample[@name='all']/Barcode[@name='all']//Pf/Read/YieldQ30",
    )
    # raw_q30 = xpathsum(tree, ".//Project[@name='all']/Sample[@name='all']/Barcode[@name='all']//Raw/Read/YieldQ30")
    pf_read1_q30 = xpathsum(
        tree,
        ".//Project[@name='all']/Sample[@name='all']/Barcode[@name='all']//Pf/Read[@number='1']/YieldQ30",
    )
    pf_read2_q30 = xpathsum(
        tree,
        ".//Project[@name='all']/Sample[@name='all']/Barcode[@name='all']//Pf/Read[@number='2']/YieldQ30",
    )
    # pf_q30_bases_pc = pf_q30 / pf_yield

    pf_qscore_sum = xpathsum(
        tree,
        ".//Project[@name='all']/Sample[@name='all']/Barcode[@name='all']//Pf/Read/QualityScoreSum",
    )
...
def get_samplesheet(rundir, file_name="SampleSheet.csv", delim=","):
    ...
    with open(rundir + "/" + file_name) as sample_sheet:
        lines = [
            line
            for line in sample_sheet.readlines()
            if not line.startswith(("#", "[")) and len(line)
        ]  # skip comments and special headers
...
def calc_undetermined(rundir):
    ...
    for lane, size in sizes.items():
        proc_undetermined[lane] = (
            float(size["u_size_of"]) / (size["u_size_of"] + size["size_of"]) * 100
        )
