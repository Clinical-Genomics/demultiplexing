...
def getsamplesfromflowcell(demuxdir, flwc):
    ...
    for sample in samples:
        ...
        sample = sample.rstrip(
            "BF"
        )  # remove the reprep (B) and reception control fail (F) letters from the samplename
        fc_samples[sample] = ""
    return fc_samples
...
def get_indexes(demuxdir, flwc):
    ...
    samplesheet_file_name = glob.glob(
        "{demuxdir}*{flowcell}/SampleSheet.csv".format(demuxdir=demuxdir, flowcell=flwc)
    )[0]
...
def make_link(demuxdir, outputdir, family_id, cust_name, sample_name, fc, index):
    ...
    for fastqfile in fastqfiles:
        ...
        if cust_name != None and family_id != None:
            ...
            try:
                print(
                    "Creating symlink for {} in {}".format(fastqfile, dest_fastqfile),
                    end="",
                )
                os.symlink(
                    fastqfile,
