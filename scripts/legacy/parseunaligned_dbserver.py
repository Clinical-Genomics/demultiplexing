...
if len(sys.argv) > 2:
    ...
    message = (
        "usage: "
        + sys.argv[0]
        + " <BASEDIRECTORYforUNALIGNED> <UNALIGNEDsubdir> <absolutepathtosamplesheetcsv> <config_file:optional>"
    )
    sys.exit(message)
...
cnx = mysql.connect(
    user=params["CLINICALDBUSER"],
    port=int(params["CLINICALDBPORT"]),
    host=params["CLINICALDBHOST"],
    passwd=params["CLINICALDBPASSWD"],
    db=params["STATSDB"],
)
cursor = cnx.cursor()
...
if str(major) + "." + str(minor) + "." + str(patch) == _VERSION_:
    ...
    exit(
        params["STATSDB"]
        + "Incorrect DB version. This script is made for "
        + str(_VERSION_)
        + " not for "
        + str(major)
        + "."
...
cursor.execute(
    """ SELECT supportparams_id FROM supportparams WHERE document_path = %s """,
...
if not cursor.fetchone():
    ...
    try:
        cursor.execute(
            """ INSERT INTO `supportparams` (document_path, systempid, systemos, systemperlv, systemperlexe, 
    ...
    cnx.commit()
    print "Support parameters from " + basedir + "Unalign*/support.txt now added to DB with supportparams_id: " + str(
        cursor.lastrowid
    )
    ...
    print "Support " + basedir + "Unaligned/support.txt" + " exists in DB with supportparams_id: " + str(
        supportparamsid
    )
...
cursor.execute(
    """ SELECT datasource_id FROM datasource WHERE document_path = %s """,
    (demultistats,),
)
if not cursor.fetchone():
    ...
    try:
        cursor.execute(
            """ INSERT INTO `datasource` (document_path, runname, rundate, machine, supportparams_id, server, time) 
    ...
    # handle warnings, if the cursor you're using raises them
    cnx.commit()
    print "Data source " + demultistats + " now added to DB with datasource_id: " + str(
        cursor.lastrowid
    )
...
for row in rows:
    ...
    if not cursor.fetchone():
        ...
        try:
            cursor.execute(
                """INSERT INTO `project` (projectname, time) VALUES (%s, %s) """,
                (project, now),
            )
        except mysql.IntegrityError, e:
            print "Error %d: %s" % (e.args[0], e.args[1])
...
for row in rows:
    ...
    if not cursor.fetchone():
        ...
        try:
            cursor.execute(
                """INSERT INTO `unaligned` (sample_id, flowcell_id, lane, yield_mb, passed_filter_pct, readcounts, 
                ...
                (
                    ...
                    passed_filter_pct,
                    Readcounts,
                    raw_clusters_per_lane_pct,
                    perfect_indexreads_pct,
                    q30_bases_pct,
                    mean_quality_score,
                    now,
                ),
        ...
        # handle warnings, if the cursor you're using raises them
        cnx.commit()
        print "Unaligned stats for sample " + samplename + " now added to DB with unaligned_id: " + str(
            cursor.lastrowid
        )
    else:
        cursor.execute(
